...
inductive path {a: Nat} (r: NatEpsNFA a): Fin r.n → Fin r.n → List (Fin a) → Prop where
  ...
  | trans (b: Fin a) (t: List (Fin a)) (p q u: Fin r.n) (h1: u ∈ r.δ p (some b)) (h2: r.path u q t): r.path p q (b::t)
  | eps (l: List (Fin a)) (p q u: Fin r.n) (h1: u ∈ r.δ p none) (h2: r.path u q l): r.path p q l
